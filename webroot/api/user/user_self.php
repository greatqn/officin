...
function user_read_base($fields="*",$condition="",$get_type="",$num="",$by_col="user_id",$order="desc",$cache="",$cache_key=""){
  ...
  dbplugin('r');
	$allow_cols=array(
		"user_id","user_email","user_name","cn_name","user_sex","birth_province","birth_city","reside_province","reside_city","user_ico","user_add_time","birth_year","birth_month","birth_day","creat_group","join_group","guest_num","integral","lastlogin_datetime","hidden_pals_id","hidden_type_id","inputmess_limit","palsreq_limit","is_pass","access_limit","access_questions","access_answers","forget_pass","introduction"
	);
  $limit=$num ? " limit $num ":"";
...
function user_self_login($email){
	require_once("foundation/module_mypals.php");
	
}
//推荐
function user_self_fir_recommend($fields="*",$userId,$pals="",$num=10){
	$fields=filt_fields($fields);
	$get_type="";
	...
	return user_read_base($fields,$condition,$get_type,$num);
}
?>