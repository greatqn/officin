...
//日志基础api函数
function blog_read_base($fields="*",$condition="",$get_type="",$num="",$by_col="edit_time",$order="desc",$cache="",$cache_key=""){
	global $tablePreStr;
	global $page_num;
	global $page_total;
	global $is_self;
...
function blog_self_count_by_uid($id){
	$result_rs=array();
	$dbo=new dbex;
  	dbplugin('r');
	global $tablePreStr;
	...
	$sql="select count(*) from $t_blog where $condition";
	$rs = $dbo->getRow($sql);
	return intval($rs[0]);
}

function blog_self_add_count($blog_id,$field){
	$dbo=new dbex;
  	dbplugin('r');
	global $tablePreStr;
	$t_blog=$tablePreStr."blog";
	$id_str=filt_num_array($blog_id);
	$condition=strpos($id_str,",") ? " log_id in ($id_str) " : " log_id = $id_str ";
	$sql="update $t_blog set $field=$field+1 where $condition";
	$dbo->exeUpdate($sql);
}
