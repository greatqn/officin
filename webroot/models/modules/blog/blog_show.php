<?php
  ...
  if($ses_uid!=getCookie('b_'.$ulog_id)){
	...
	if($ulog_id){
		...
		//$user_info=api_proxy("user_self_by_uid","*",$blog_row["user_id"]);
		$str_user = "<a href=\"home.php?h=".$blog_row["user_id"]."\" target=\"_blank\">".$blog_row["user_name"]."</a>";
		$str_title=str_replace("{holder}",$str_user,$b_langpackage->b_scan);
		
	    if($is_self=='N'){
	  	require("servtools/menu_pop/trans_pri.php");
	  	$is_show=check_pri($blog_row['user_id'],$blog_row['privacy']);
	  	//Logs::addLog("blog_show 1:is_show:$is_show");
	  }
	}
	//控制按钮显示
	...
	if(empty($blog_row)){
		...
		$is_show=0;
		$show_error="";
		//Logs::addLog("blog_show 2:empty:$is_show");
	}
?>