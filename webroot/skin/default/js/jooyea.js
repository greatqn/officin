...
function AddContentImg(ImgName,classId){
	if(editor){
		...
		editor.insertHtml("<br><IMG src='./"+ImgName+"' onload='parent.fixImage(this,420,0)' width='"+width_value+"' /><br>");
	}
}
...
function check_code_size(string_data){
	...
	//喜欢按钮。0.collect,2.agree,3.oppose,4.thank;
	function like_click(type_id,blog_id,isshowtxt,isadd){
		var ajax_act_share=new Ajax();
		ajax_act_share.getInfo("do.php?act=like_action&s_type="+type_id+"&blog_id="+blog_id,"post","app","",function(c){like_click_callback(c,type_id,blog_id,isshowtxt,isadd)});
	}
	function like_click_callback(c,type_id,blog_id,isshowtxt,isadd){
		var ctl_id = "btn_like_"+blog_id+"_"+type_id;
		var a_md = $(ctl_id);
		var add_success = c;
		if(add_success==0){
			alert("已经点过啦。");
		}else{
			//a_md.setAttribute("txtdate","false");
			like_click_add(a_md);
			var key = blog_id +"_" + type_id;
			MYCARD[key] = add_success;
		}
		
		//i.setAttribute("novalidate","true");		
		
	}
	function show_liketxt(){
	
	}
	function like_click_add(a_md){
		//var a_md = $("btn_like_"+mod_id+"_"+type_id);
		//console.log(a_md);
		var num = a_md.getAttribute("cnum");
		a_md.innerHTML = a_md.innerHTML.replace(num,parseInt(num)+1); 
		//var ctl_id = "btn_like_"+mod_id+"_"+type_id;
		//showMenu({'ctrlid':ctl_id, 'pos':'auto','menuid':'bus_card_pane','blog_id':mod_id});
		//console.log(ctl_id);
	}