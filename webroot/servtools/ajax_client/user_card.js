...
function check_mycard(user_id,menuObj){
	if(isUndefined(MYCARD[user_id])){
		var get_album=new Ajax();
		get_album.set_async(false);
		get_album.getInfo("do.php","get","app","act=show_business_card&uid="+user_id,function(c){
			menuObj.innerHTML = c;
			MYCARD[user_id] = c;
		});
		
	}else{
		menuObj.innerHTML=MYCARD[user_id];
	}
}
function showMenu(v) {
	...
	var user_id = isUndefined(v['user_id']) ? 0 : v['user_id'];
	check_mycard(user_id,menuObj);
