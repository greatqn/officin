...
class Logs
{
     ...
     * 记录url和post数据
     */
    public static function addhttp()
    {
    	self::addLog("GET: ".self::_getUrl());
    	self::addLog("POST: ".self::_postData());
    }
    
    public static function init($logpath)
    {
        //默认路径为当前路径
        //$_SERVER['DOCUMENT_ROOT']docs\log
        self::$_filepath = $logpath;// . date("Y/m", time());
