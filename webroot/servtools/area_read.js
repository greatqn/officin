 ...
 * jQuery多级联动菜单
 * 
 * Author:	greatqn <greatqn@gmail.com>
 * 
 */
function LinkageSelect(options) {
	...
	function _bind(element , value) {
		...
		element.change(function() {
			...
			if (settings.all_name) {
				var m_allname = "";
				for (var el_id in bindEls) {
					m_allname += bindEls[el_id].element.find("option:selected").text();
					m_allname += "-";
				}
				$(settings.all_name).val(m_allname.substring(0,m_allname.length-1));
			}
	...
	function _fill(element , key , value) {
		element.empty();
		element.append('<option value="">'+element.attr("defvalue")+'</option>');
	...
	function _setval(value){
		...
		if (settings.field_name) {
			$(settings.field_name).val(value);
		}
		var defs = new Array();
		if (settings.ajax_me) {
			$.getJSON(settings.ajax_me , {key:value} , function(json) {
				var arr = json[0].code.split("_");
				var arr_key = settings.root;
				for(var defv in arr) {
					_fill(bindEls[defv].element,arr_key,arr[defv]);
					arr_key = arr_key + "," + arr[defv];
				}
			})
		}
	}
